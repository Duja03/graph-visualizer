...
class CsvDataSourcePlugin(DataSourcePlugin):
    ...
    def parameters(self) -> Dict[str, str]:
        return {
            ...
            "edge_column": "Column referencing another row's id (default: connected_to). "
                           "Edge attributes go in columns prefixed with 'edge_' (e.g. edge_weight, edge_type).",
        }
    ...
    def load(self, **kwargs: Any) -> Graph:
        ...
        edge_column = kwargs.get("edge_column") or "connected_to"
        edge_counter = 0
        ...
        with open(p, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                for col, val in row.items():
                    if col == "id" or col == edge_column or col.startswith("edge_"):
                        continue
        ...
        for row in rows:
            ...
            for col, val in row.items():
                if col.startswith("edge_"):
                    attr_name = col[len("edge_"):]
                    typed = _parse_typed_value(val)
                    if typed is not None:
                        edge_attributes[attr_name] = typed
